...
let allSubmissions = [], allAiAds = [], allClients = [];
...
function activateClient() {
  ...
  if (!hasWeb && !hasAi) { errEl.textContent = 'Please select at least one service.'; errEl.style.display = 'block'; return; }

  const services = (hasWeb && hasAi) ? 'both' : hasWeb ? 'website' : 'aiads';
  ...
  window[cbName] = function(res) {
    ...
    if (res && res.success) {
      ...
      document.getElementById('svcWebsite').checked = true; document.getElementById('svcAiAds').checked = false;
      const label = services === 'both' ? 'Website + AI Ads' : services === 'aiads' ? 'AI Ads' : 'Website';
